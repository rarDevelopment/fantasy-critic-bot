...
exports.updateLastCheckTime = async function (lastCheckTimeData) {
    await LastCheckTime.findOne({ checkType: lastCheckTimeData.checkType })
        .exec()
...
exports.updateMasterGameList = async function (gameData) {
    for (const g of gameData) {
        await MasterGameListItem.findOne({ masterGameID: g.masterGameID })
            .exec()
            .then((gameFound) => {
                if (!gameFound) {
